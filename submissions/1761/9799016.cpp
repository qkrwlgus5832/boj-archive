...
pair<long long, int> rPtks(int x, int y) {
	long long result = 0;
	int y1 = y;
	for (int i = 0; i < depth1[y]- depth1[x]; i++) {
		result = result + v2[y1].second;
		y1 = v2[y1].first;

	}
	return make_pair(result,y1);
}
...
int main() {
	...
	for (int i = 0; i < M; i++) {
		...
		if (depth1[tmp] < depth1[tmp2]) {
			pair <long long, int> d = rPtks(tmp, tmp2);
			result = result + d.first;
			tmp2 = d.second;
		}
		else if (depth1[tmp] > depth1[tmp2]) {
			pair <long long, int> d = rPtks(tmp2, tmp);
			result = result + d.first;
			tmp = d.second;
		}
		while (1) {
