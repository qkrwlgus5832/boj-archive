...
vector <pair <int, int>> segmenttree[300001];
...
//int divideandconquer(int rangestart, int rangeend, int n) {
//	queue <pair <int, int>> q;
//
//	q.push(make_pair(rangestart, rangeend));
//	int count = 0;
//	
//	while (!q.empty()) {
//		int s = q.size();
...
//				result = max(result, (long long)histogram[front.first]);
//				continue;
//			}
//			count++;
//			pair <int, int> queryresult = querymin(0, front.first, front.second, 0, n - 1);
//			result = max(result, (long long)(front.second - front.first + 1) * queryresult.first);
//			if (front.first <= queryresult.second -1)
//				q.push(make_pair(front.first, queryresult.second - 1));
//			if (queryresult.second+1 <= front.second)
//				q.push(make_pair(queryresult.second+1, front.second));
//		
//		}
//		
//	}
...
void divideandconquer(int rangestart, int rangeend, int n) {
	if (rangestart > rangeend) {
		return;
	}
	else if (rangestart ==  rangeend) {
		result = max(result, (long long )histogram[rangestart]);
		return;
	}
	pair <int, int> queryresult = querymin(0, rangestart, rangeend, 0, n-1);
	result = max(result, (long long) (rangeend - rangestart + 1) * queryresult.first);
	
	divideandconquer(rangestart, queryresult.second - 1, n);
	divideandconquer(queryresult.second + 1, rangeend, n);
