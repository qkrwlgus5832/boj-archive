...
int getArea(pair <int, int> sticker1, pair <int, int> sticker2) { // 두개의 스티커로 구할 수 있는 최대 넓이를 구한다.
	if (sticker1.first + sticker2.first > H || sticker2.second > W) { // 최대 높이, 최대 너비를 넘어가는 경우 0을 리턴한다.
		if (sticker1.first > H || sticker1.second + sticker2.second > W) {
			return 0;
		}
	}
...
void combination(int index, vector <int> v) { // 조합으로 중복되지 않는 2개의 스티커를 구한다.
	if (v.size() == 2) {
		getAnswer(v);
		return;
	}
