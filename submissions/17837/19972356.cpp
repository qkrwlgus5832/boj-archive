...
bool move() {
	for (int i = 0; i < horses.size(); i++) {
		...
		if (graph[ni][nj] == 0) {
			...
			for (int i1 = 0; i1 < v[horses[i].x][horses[i].y].size(); i1++) {
				if (v[horses[i].x][horses[i].y][i1] == i) {
					tmp.push_back(v[horses[i].x][horses[i].y][i1]);
					count = 1;
				}
				else if (count> 0) {
					count++;
			...
			for (int i1 = 0; i1 < v[horses[i].x][horses[i].y].size(); i1++) {
				if (v[horses[i].x][horses[i].y][i1] == i) {
					tmp.push_front(v[horses[i].x][horses[i].y][i1]);
					count = 1;
				}
				else if (count > 0) {
					count++;
		...
		for (int j = 0; j < count; j++) {
			v[horses[i].x][horses[i].y].pop_back();
		}
		for (int j = 0; j < tmp.size(); j++) {
			v[ni][nj].push_back(tmp[j]);
			horses[tmp[j]].x = ni;
			horses[tmp[j]].y = nj;
