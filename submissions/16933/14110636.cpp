...
void qpush(int x, int y, int dd, int flag, int count, bool issunny) {
	...
	for (int i = 0; i < 4; i++) {
		...
		if (i < 4 && (graph[nx][ny] != dd || check[nx][ny][flag][issunny] == true))
			continue;

		tmp.x = nx; tmp.y = ny; tmp.flag = flag; tmp.count = count + 1; tmp.issunny = issunny;
		q.push(tmp);
		check[nx][ny][flag][issunny] = true;
	}
...
int BFS(int x, int y) {
	...
	while (!q.empty()) {
		...
		for (int i = 0; i < ss; i++) {
			...
			if (check[x][y][flag][!issunny] == false) {
				check[x][y][flag][!issunny] = true;
				tmp.x = x; tmp.y = y; tmp.flag = flag; tmp.count = count + 1; tmp.issunny = !issunny;
				q.push(tmp);
			}
		}
