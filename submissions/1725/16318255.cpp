		...
		if (st.size() == 0)
			...
			while (st.size()) {
				if (st.top().first > histarea[i]) {
					...
					histMaxArea[i].first = count + 1 + histMaxArea[st.top().second].first;
					st.push({ histarea[i],i });
					flag = true;
					break;
				}
			...
			if (flag == false) {
				if (st.size() == 0)
					aimindex = 0;
				histMaxArea[i].first = i - aimindex;
				st.push({ histarea[i], i });
			}
		}
	}
		...
		if (st.size() == 0)
			...
			while (st.size()) {
				if (st.top().first > histarea[i]) {
					...
					histMaxArea[i].second = count + 1 + histMaxArea[st.top().second].second;
					st.push({ histarea[i],i });
					flag = true;
					break;
				}
			...
			if (flag == false) {
				if (st.size() == 0)
					aimindex = histarea.size() - 1;
				histMaxArea[i].second = abs(i - aimindex);
				st.push({ histarea[i], i });
			}
		}
	}
