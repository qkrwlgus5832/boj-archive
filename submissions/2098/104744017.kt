...
class `외판원 순회` {
    ...
    class Solution {
        ...
        private fun bfs(start: Int, graph: Array<MutableList<Pair<Int, Int>>>) {
            ...
            val dp = Array((pow(2.toDouble(), 16.toDouble()) + 1).toInt()) { IntArray(graph.size) { -1 } }
            ...
            while (queue.isNotEmpty()) {
                ...
                for (next in graph[current]) {
                    ...
                    if (dp[nextChecked][next.first] == -1 || dp[nextChecked][next.first] > cost + next.second) {
                        ...
                        queue.add(Node(next.first, nextChecked, cost + next.second, count + 1))
                    }
