...
int recursion(int x, int y, char dir, int distance) {
	...
	if (dir == 'U') {
		...
		else if (x-1 >= 1 && graph[x - 1][y] == '*' && check[x-1][y][0] >=1)
			return -1;
		else
			return distance;
	}
		...
		else if (y + 1 <= M && graph[x][y + 1] == '*' && check[x][y + 1][1] >=1)
			return -1;
		else
			return distance;
	}
		...
		if (x + 1 <= N && check[x+1][y][2] == 0 &&  graph[x+1][y] !='C')
			return recursion(x + 1, y, dir, distance + 1);
		else if (x + 1 <= N && graph[x + 1][y] == '*' && check[x + 1][y][2] == 0)
			return -1;
		else
			return distance;
	}
		...
		if (y - 1 >= 1 && check[x][y-1][3] == 0 && graph[x][y-1] !='C')
			return recursion(x, y - 1, dir, distance + 1);
		else if (y - 1 >= 1 && graph[x][y - 1] == '*' && check[x][y-1][3] == 0)
			return -1;
		else
			return distance;
