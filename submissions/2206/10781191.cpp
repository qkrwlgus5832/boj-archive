...
void qpush(int x, int y, int dd, int flag, int count) {
	if (dd == 1)
		flag = 1;
	if (x + 1 < N && dir[x + 1][y] == dd && check[x+1][y] == false)
		q.push(make_pair(make_pair(x + 1, y), make_pair(flag, count+1)));
	if (y + 1 < M && dir[x][y+1] == dd && check[x][y+1] == false)
		q.push(make_pair(make_pair(x , y+1), make_pair(flag, count+1)));
	if (x - 1 >=0 && dir[x - 1][y] == dd && check[x-1][y] == false)
		q.push(make_pair(make_pair(x - 1, y), make_pair(flag, count+1)));
	if (y- 1 >=0 && dir[x][y-1] == dd && check[x][y-1] == false)
		q.push(make_pair(make_pair(x , y-1), make_pair(flag, count+1)));
