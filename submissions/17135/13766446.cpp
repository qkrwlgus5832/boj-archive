...
int game(int enemy[][16]) {
	...
	for (int t = 0; t < N; t++) {
		for (int aindex = 0; aindex < archer.size(); aindex++) {
			...
			for (int j = 0; j < M; j++) {
				for (int i =0; i < N; i++) {
					int distance = getdistance(i , j, N, archer[aindex]);
			...
			if (flag == true) {
				if (check[attackenemy.x][attackenemy.y] == false) {
					count++;
					check[attackenemy.x][attackenemy.y] = true;
				}
