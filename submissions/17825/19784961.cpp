...
void recursion(int i1, int value) {
	...
	for (int i = 0; i < 4; i++) {
		...
		int tmploc = horses[i].first;
		int tmpdir = horses[i].second;

		pair <int,int> changedLocation = go(tmploc, input[i1], false, tmpdir);

		int location = changedLocation.first;
	
		 if (location == 'E') {
			horses[i].first = location;
			horses[i].second = changedLocation.second;
			recursion(i1 + 1, value);
			horses[i].first = tmploc;
			horses[i].second = tmpdir;
		}
		 else if (location > 0) {
			 horses[i].first = location;
			 horses[i].second = changedLocation.second;
			 recursion(i1 + 1, value + location);
			 horses[i].first = tmploc;
			 horses[i].second = tmpdir;
