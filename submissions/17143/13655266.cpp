	...
	while (!q.empty()) {
		...
		if (count == speed[sharkindex]) {
			if (ocean[fronti][frontj] < size) {
				...
				if (ocean[fronti][frontj] != 0 && check[oceansharkindex[fronti][frontj]] == true)
					die[oceansharkindex[fronti][frontj]] = true; 
				oceansharkindex[i][j] = 0;
				ocean[fronti][frontj] = size;
				oceansharkindex[fronti][frontj] = sharkindex;
			}
			else {
				if (check[oceansharkindex[fronti][frontj]] == true) {
					oceansharkindex[i][j] = 0;
					die[sharkindex] = true;
				}
				else {
...
int fishing(int fishingcount){
	...
	if (minsharkindex != -1) {
		oceansharkindex[shark[minsharkindex].first.first][shark[minsharkindex].first.second] = 0;
